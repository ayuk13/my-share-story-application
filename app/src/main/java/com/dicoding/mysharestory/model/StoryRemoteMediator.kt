...
import com.dicoding.mysharestory.repository.db.RemoteKeys
...
class StoryRemoteMediator(
    ...
    override suspend fun load(
        ...
        try {
            Log.d(TAG, "load: $apiToken")
            val responseStory = apiService.getAllStories(
                "Bearer $apiToken",
                page,
                state.config.pageSize
            )
            val listStory = responseStory.listStory as MutableList<Story>

            val endOfPaginationReached = listStory.isEmpty()
            database.withTransaction {
                if (loadType == LoadType.REFRESH) {
                    database.remoteKeysDao().deleteRemoteKeys()
                    database.storiesDao().deleteStories()
                }
    ...
    private suspend fun getRemoteKeyForLastItem(state: PagingState<Int, Story>): RemoteKeys? {
        return state.pages.lastOrNull { it.data.isNotEmpty() }?.data?.lastOrNull()?.let { data ->
            database.remoteKeysDao().getIdRemoteKeys(data.id)
        }
    }
    ...
    private suspend fun getRemoteKeyClosestToCurrentPosition(state: PagingState<Int, Story>): RemoteKeys? {
        return state.anchorPosition?.let { position ->
            state.closestItemToPosition(position)?.id?.let { id ->
                database.remoteKeysDao().getIdRemoteKeys(id)
            }
        }
    }
